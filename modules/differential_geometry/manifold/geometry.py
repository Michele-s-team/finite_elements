...
def H_Nabla_omega(omega, nu=None):
    return (1.0 / 2.0 * sqrt_detg(omega, nu) * g_c(omega, nu)[i, j] * Nabla_f(omega, nu, omega, nu)[j, i])
...
def n_c_r(mesh, c_r, omega, nu=None):
    return as_tensor((Nt_c_r(mesh, c_r, omega, nu))[k] / sqrt(g(omega, nu)[i, j] * (Nt_c_r(mesh, c_r, omega, nu))[i] * (Nt_c_r(mesh, c_r, omega, nu))[j]), (k))
...
def from_tangent_normal_to_3D_space(omega, v_t, v_n, nu=None):
    return from_tangent_to_3D_space(omega, nu, v_t) + v_n * normal(omega, nu)
