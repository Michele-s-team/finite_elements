...
class u_exact_expression( UserExpression ):
    def eval(self, values, x):
        # values[0] = 1 + np.cos( x[0] - x[1] ) - sin( x[1] )
...
class v_exact_expression( UserExpression ):
    def eval(self, values, x):
        ...
        values[0] =  2 *(np.pi) *np.cos(2 *(np.pi) *((x[0]) - (x[1]))**2) * np.cos(2 *(np.pi) *((x[0]) + (x[1]))) + 4 *(np.pi) *(-(x[0]) + (x[1]))* sin(2 *(np.pi) * ((x[0]) - (x[1]))**2) * sin(2 * (np.pi) * ((x[0]) + (x[1])))
        values[1] = 2 * (np.pi) * np.cos(2* (np.pi) * ((x[0]) - (x[1]))**2) * np.cos(2 * (np.pi) * ((x[0]) + (x[1]))) + 4* (np.pi) * ((x[0]) - (x[1])) * sin(2 *(np.pi) *((x[0]) - (x[1]))**2) * sin(2 * (np.pi)*  ((x[0]) + (x[1])))
...
class laplacian_u_exact_expression( UserExpression ):
    def eval(self, values, x):
        # values[0] = -2 * np.cos( x[0] - x[1] ) + sin( x[1] )
        values[0] = 8 *(np.pi)* (-(np.pi)* (1+4* (x[0]-(x[1]))**2) * np.cos(2* (np.pi)* (x[0]-(x[1]))**2)-sin(2* (np.pi) *(x[0]-(x[1]))**2))* sin(2* (np.pi)* (x[0]+(x[1])))
...
F_N = rpam.parameters['alpha'] / rpam.parameters['r_mesh'] * (bgeo.facet_normal[i] * fsp.v[i] - bgeo.facet_normal[i] * fsp.v_exact[i]) * bgeo.facet_normal[j] * fsp.nu_v[j] * rmsh.ds
