...
class u_exact_mesh_1_expression(UserExpression):
    def eval(self, values, x):
        # test case 1
        values[0] = 1 + x[0]**2 + 2 * (rmsh.parameters['shape_coordinates'][0][1])**2

    def value_shape(self):
        return (1,)


class grad_u_exact_mesh_1_expression(UserExpression):
    def eval(self, values, x):
        # test case 1
        values[0] = 2 * x[0]
...
class laplacian_u_exact_mesh_1_expression(UserExpression):
    def eval(self, values, x):
        # test case 1
        values[0] =  2
...
F = (fsp.u[1].dx(i) * fsp.nu_u[1].dx(i) + fsp.f[1] * fsp.nu_u[1]) * rmsh.dx_mesh[1] \
    - bgeo.facet_normal[1][i] * (fsp.u[1].dx(i)) * fsp.nu_u[1] * rmsh.ds_mesh[1]['ds']
