...
class u_exact_mesh_1_expression(UserExpression):
    def eval(self, values, x):
        ...
        # test case 2) 
        # values[0] = 1.0/12.0 * rmsh.lmsh.parameters['r']**2 * (-9.0 * np.cos(x[0] / rmsh.lmsh.parameters['r']) + np.cos(3.0 * x[0] / rmsh.lmsh.parameters['r']) + 4.0 * (5.0 + np.cos(2.0 * x[0] / rmsh.lmsh.parameters['r'])) * np.sin(x[0] / rmsh.lmsh.parameters['r']))
