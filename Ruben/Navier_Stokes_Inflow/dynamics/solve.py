...
import dolfin
from fenics import *
from fenics import XDMFFile, project, Expression, pi
import importlib
import sys
from solve_steady import solve_steady
...
'''
# add the perturbations onto the steady state
_,_, u_star, p_star = solve_steady()
steady_u = Function(fsp.Q_v)
steady_u.interpolate(u_star)
steady_p = Function(fsp.Q)
steady_p.interpolate(p_star)
fsp.sigma_n_32.assign(steady_p)
fsp.phi.assign(steady_p)  # used as p^{n+1}
pert = Expression(
    ("eps*sin(2*pi*x[0])", "eps*sin(2*pi*x[1])"),
    degree=2, eps=1e-3
)
u_pert = project(pert, fsp.Q_v)
steady_u.vector().axpy(1.0, u_pert.vector())  # add perturbation to steady state
fsp.v_n_1.assign(steady_u)
fsp.v_n_2.assign(steady_u)
# set initial pressure via your existing fields
...
err_u = sqrt(assemble(dot(u_diff, u_diff) * rmsh.dx))
