...
def build_J(rB):
    ...
    B3AAA, B3AAB, B3BBA, B3BBB, B2BBA = B3(RA,RA,RA), B3(RA,RA,rB), B3(rB,rB,RA), B3(rB,rB,rB), B3(rB, rB, RA)
    fAA = 1.0 +  B2AA*cA0 + B3AAA*cA0**2 + B3AAB*cA0*cB0
    fBB = 1.0 + B2BB*cB0 + B3BBB*cB0**2 + B3BBA*cA0*cB0 
    fAB = B2AB*cA0 + B3AAB*cA0**2 + B3BBA*cA0*cB0
    fBA = B2BA*cB0 + B2BBA*cB0**2 + B3AAB*cA0*cB0
...
rB_list = np.linspace(0.7, 1.1, 100)
rB0 = rB_list[0]
...
eigvectors = []
eigvecs = np.zeros((len(rB_list), 2*nPts), dtype=complex)

for i, rB in enumerate(rB_list):
    Jsys    = build_J(rB)
    J_dense = Jsys.toarray()
    vals, vecs= np.linalg.eig(J_dense.T)
    ...
    idx_max = np.argmax(reals_full)
    v = vecs[:, idx_max]
    v /= np.linalg.norm(v)
    eigvecs[i, :] = v
np.savez('largest_eigenmodes.npz', rB=rB_list, eigvecs=eigvecs) 
