...
F_tot = [assemble(phys.force_t(omega_output, mu_output, vp.kappa, fsp.sigma, bgeo.n_circle(omega_output))[i]* bgeo.sqrt_deth_circle( fsp.omega, rmsh.c_r ) * (1.0 / rmsh.r)* rmsh.ds_r) for i in range(0, 3)]
...
print(f"Tangential force on the ring = {col.Fore.CYAN}{F_tot}{col.Style.RESET_ALL}")
#print(f"Normal force on the ring = {col.Fore.CYAN}{N_force(mu_output, kappa)}{col.Style.RESET_ALL}")
