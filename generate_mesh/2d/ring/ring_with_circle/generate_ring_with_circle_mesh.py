...
run it with
    python3 generate_ring_with_circle_mesh.py [resolution] [output directory]
example:
    clear; clear; SOLUTION_PATH="solution"; rm -rf $SOLUTION_PATH; mkdir $SOLUTION_PATH; python3 generate_ring_with_circle_mesh.py 0.1 $SOLUTION_PATH
'''
...
circle_R = model.add_circle(c_R, R, mesh_size=resolution)
circle_rho = model.add_circle(c_r, rho, mesh_size=resolution)
