...
for i in range(rpam.parameters['N']):
    ...
    if cal.point_in_box(coordinate, [[0, rpam.parameters['L']], [0, rpam.parameters['h']]]):
        ...
        shape_coordinates.append(shape_parametric_form(i/rpam.parameters['N']))
...
mesh_0_metadata['shape_id'] = rpam.parameters['shape_id']
...
mesh_1_metadata['line_id'] = rpam.parameters['shape_id']
...
#add shape lines
msh.tag_physical_object([lines[i] for i in range(4, 4 + rpam.parameters['N'])], rpam.parameters['shape_id'], gmsh.model, 'shape_loop')
...
msh.tag_physical_object(surfaces[0], rpam.parameters['sub_mesh_0_1_id'], gmsh.model, 'square_minus_shape_surface')
msh.tag_physical_object(surfaces[1], rpam.parameters['sub_mesh_0_0_id'], gmsh.model, 'shape_surface')
...
msh.sorted_boundary_points(
    ...
    [rpam.parameters['shape_id']],
    os.path.join(output_directory_mesh_0, 'boundary_points_id_' + str(rpam.parameters['shape_id']) + '.csv'))
...
if n_vertices_on_shape != rpam.parameters['N']:
...
msh.genereate_line_mesh(0, shape_length, rpam.parameters['N'],
                        rpam.parameters['shape_id'], rpam.parameters['vertex_l_id'], rpam.parameters['vertex_r_id'],
                        x_m=None,
                        vertex_m_id=None,
