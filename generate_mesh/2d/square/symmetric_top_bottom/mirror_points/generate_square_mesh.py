<<<<<<< HEAD:generate-mesh/2d/symmetric-square-circle/mirror_points/generate_square_mesh.py
'''
...
while True:
    mods = os.path.join(current, "modules")
    if os.path.isdir(mods):
        project_root = current
        break
    parent = os.path.dirname(current)
    if parent == current:
        raise RuntimeError("could not find 'modules/' in any parent")
    current = parent
...
mesh = msh.read_mesh(output_dir + "/triangle_mesh.xdmf")
io.print_vertices_to_csv_file(mesh, output_dir + "/vertices.csv")
>>>>>>> origin/master:generate_mesh/2d/square/symmetric_top_bottom/mirror_points/generate_square_mesh.py
