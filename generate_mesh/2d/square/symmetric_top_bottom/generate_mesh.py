...
class generate_mesh:
    ...
    def generate_half_square_mesh(self, L, h, r, c_r = [0.0, 0.0, 0.0]):
        ...
        half_rectangle_points = [self.model.add_point( (L/2, 0, 0), mesh_size=self.resolution*(min(L,h)/r) ),
                    self.model.add_point( (L/2, h/2, 0), mesh_size=self.resolution*(min(L,h)/r) ),
                    self.model.add_point( (-L/2, h/2, 0), mesh_size=self.resolution*(min(L,h)/r) ),
                    self.model.add_point( (-L/2, 0, 0), mesh_size=self.resolution*(min(L,h)/r) ),
                    ]
        
        half_circle_points = [self.model.add_point( (-r*np.cos(np.pi*i/N), r*np.sin(np.pi*i/N), 0), mesh_size=self.resolution ) for i in range(N+1)]
    ...
    def generate_square_mesh(self, L, h, r, c_r = [0.0, 0.0, 0.0]):
        ...
        my_points = [self.model.add_point( (L/2, h/2, 0), mesh_size=self.resolution*(min(L,h)/r) ),
                    self.model.add_point( (-L/2, h/2, 0), mesh_size=self.resolution*(min(L,h)/r) ),
                    self.model.add_point( (-L/2, -h/2, 0), mesh_size=self.resolution*(min(L,h)/r) ),
                    self.model.add_point( (L/2, -h/2, 0), mesh_size=self.resolution*(min(L,h)/r) )]
        ...
        circle_r = self.model.add_circle( c_r, r, mesh_size=self.resolution )
    ...
    def generate_ring_mesh(self, r, R, c_r=[0, 0, 0], c_R=[0, 0, 0]):
        ...
        circle_r = self.model.add_circle(c_r, r, mesh_size=self.resolution)
        circle_R = self.model.add_circle(c_R, R, mesh_size=self.resolution*(R/r))
