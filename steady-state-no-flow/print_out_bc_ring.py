...
print(
    f"\t<<(n^i \omega_i - psi )^2>>_[partial Omega R] = {col.Fore.RED}{msh.difference_wrt_measure( (rmsh.n_circle( omega_output ))[i] * omega_output[i], vp.omega_R, rmsh.ds_R ):4e}{col.Style.RESET_ALL}" )
print( f"\t<<[mu - H(omega)]^2>>_partial Omega = {col.Fore.RED}{msh.difference_on_boundary( mu_output, project(geo.H(omega_output), fsp.Q_z) ):4e}{col.Style.RESET_ALL}" )
print(
    f"\t<<|nu_i - \partial_i mu|^2>>_partial Omega = {col.Fore.RED}{msh.difference_wrt_measure(project(sqrt((nu_output[i] - (mu_output.dx(i))) * (nu_output[i] - (mu_output.dx(i)))), fsp.Q_z), project(Constant(0), fsp.Q_z), rmsh.ds):4e}{col.Style.RESET_ALL}" )
...
print( f"\t<<[tau - Nabla^i nu_i]^2>>_partial Omega =  {col.Fore.RED}{msh.difference_on_boundary( tau_output, project( geo.g_c(fsp.omega)[i, j] * geo.Nabla_f(nu_output, omega_output)[i, j], fsp.Q_z ) ):4e}{col.Style.RESET_ALL}" )
