...
F_omega = (- z * Nabla_v( nu_omega, fsp.omega )[i, i] - fsp.omega[i] * nu_omega[i]) * geo.sqrt_detg( fsp.omega ) * dx \
          + ((geo.n_circle( fsp.omega ))[i] * g( fsp.omega )[i, j] * z * nu_omega[j]) * geo.sqrt_deth_circle( fsp.omega, rmsh.c_r ) * (1.0 / rmsh.r) * rmsh.ds_r \
          + ((geo.n_circle( fsp.omega ))[i] * g( fsp.omega )[i, j] * z * nu_omega[j]) * geo.sqrt_deth_circle( fsp.omega, rmsh.c_R ) * (1.0 / rmsh.R) * rmsh.ds_R
...
F_nu = (fsp.nu[i] * nu_nu[i] + fsp.mu * Nabla_v( nu_nu, fsp.omega )[i, i]) * geo.sqrt_detg( fsp.omega ) * dx \
       - ((geo.n_circle( fsp.omega ))[i] * g( fsp.omega )[i, j] * fsp.mu * nu_nu[j]) * geo.sqrt_deth_circle( fsp.omega, rmsh.c_r ) * (1.0 / rmsh.r) * rmsh.ds_r \
       - ((geo.n_circle( fsp.omega ))[i] * g( fsp.omega )[i, j] * fsp.mu * nu_nu[j]) * geo.sqrt_deth_circle( fsp.omega, rmsh.c_r ) * (1.0 / rmsh.R) * rmsh.ds_R

F_N = alpha / r_mesh * ( \
            + (((geo.n_circle( fsp.omega ))[i] * fsp.omega[i] - omega_r) * ((geo.n_circle( fsp.omega ))[k] * g( fsp.omega )[k, l] * nu_omega[l])) * geo.sqrt_deth_circle( fsp.omega, rmsh.c_r ) * (1.0 / rmsh.r) * rmsh.ds_r \
            + (((geo.n_circle( fsp.omega ))[i] * fsp.omega[i] - omega_R) * ((geo.n_circle( fsp.omega ))[k] * g( fsp.omega )[k, l] * nu_omega[l])) * geo.sqrt_deth_circle( fsp.omega, rmsh.c_R ) * (1.0 / rmsh.R) * rmsh.ds_R \
    )
