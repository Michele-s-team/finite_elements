...
print(
    f"\t<<|omega - omega_exact|^2>>_partial Omega = {col.Fore.RED}{np.sqrt( assemble( (bgeo.facet_normal[i] * omega_output[i] - bgeo.facet_normal[i] * fsp.omega_exact[i]) ** 2 * rmsh.ds ) / assemble( Constant( 1 ) * rmsh.ds ) ):.{io.number_of_decimals}e}{col.Style.RESET_ALL}" )
