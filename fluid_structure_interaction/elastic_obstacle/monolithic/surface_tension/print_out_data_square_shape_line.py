...
def print_data(step):
            ...
            f"{msh.average_wrt_measure( sqrt( ( - cont.pressure_scale * 2 * rpam.parameters['sigma'] * msh.average(fsp.mu_n * ela.detF(fsp.u_n) * ela.G(fsp.u_n)[k, i]) * bgeo.facet_normal[0](vp.sub_mesh_0_label)[k]) * ( - cont.pressure_scale * 2 * rpam.parameters['sigma'] * msh.average(fsp.mu_n * ela.detF(fsp.u_n) * ela.G(fsp.u_n)[l, i]) * bgeo.facet_normal[0](vp.sub_mesh_0_label)[l]) ), rmsh.ds_mesh[0]['dS_shape'] ):.{rpam.parameters['print_out_digits']}e}"
        }])
