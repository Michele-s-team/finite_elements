...
fieldnames_bcs = [ \
    ...
    '<<(w_bar - w_bar_lr)^2>>_{partial Omega}', \
    '<<(n^i Nabla_i phi)^2>>_{partial Omega l}', \
    '<<(phi - phi_r)^2>>_{partial Omega r}', \
    '<<|X_n_12 - X_n_12_l|^2>>_{partial Omega l}', \
    '<<|X_n_12 - X_n_12_r|^2>>_{partial Omega r}' \
    ]
...
def print_bcs(psi):
    ...
    writer_bcs.writerows( [{ \
        ...
        fieldnames_bcs[2]: \
            f"{msh.difference_wrt_measure( w_bar_dummy, rpam.parameters['w_bar_lr'], rmsh.ds ):.{io.number_of_decimals}e}",\
        fieldnames_bcs[3]: \
            f"{msh.abs_wrt_measure( (bgeo.n_lr( psi_n_12_dummy, nu_n_12_dummy ))[i] * (fsp.phi.dx( i )), rmsh.ds_l ):.{io.number_of_decimals}e}",\
        fieldnames_bcs[4]: \
            f"{msh.difference_wrt_measure( phi_dummy, 0, rmsh.ds_r ):.{io.number_of_decimals}e}",\
        fieldnames_bcs[5]: \
            f"{msh.abs_wrt_measure(sqrt((X_n_12_dummy[0] - rpam.parameters['X_n_12_l'][0])**2 + (X_n_12_dummy[1] - rpam.parameters['X_n_12_l'][1])**2), rmsh.ds_l):.{io.number_of_decimals}e}",\
        fieldnames_bcs[6]: \
            f"{msh.abs_wrt_measure(sqrt((X_n_12_dummy[0] - rpam.parameters['X_n_12_r'][0])**2 + (X_n_12_dummy[1] - rpam.parameters['X_n_12_r'][1])**2), rmsh.ds_r):.{io.number_of_decimals}e}",\
        }] )
    csvfile_bcs.flush()

# msh.abs_wrt_measure(sqrt((prout.X_output[0] - rpam.parameters['X_l'][0])**2 + (prout.X_output[1] - rpam.parameters['X_l'][1])**2), rmsh.ds_l):.{io.number_of_decimals}e
...
def print_solution(psi, step, t):
    ...
    files.xdmffile_X_n_12.write( z_n_12_output, t - vp.dt / 2.0 )
    files.xdmffile_nu_n_12.write( omega_n_12_output, t - vp.dt / 2.0 )
